...
class ReplyVM {
	 ...
	 * Update ViewModel
	 */
	public async render(track: Track | undefined, queue: Queue<Track>, paused: boolean): Promise<void>
	{
		if (track)
			(await this._replyMessage).edit(await this.busyPlayer(track, queue, paused));
		else
			(await this._replyMessage).edit(await this.emptyPlayer());
	}
	...
	{
		return {
			content: "Nothing playing right now, type `/play <song>` to add a song to the queue.",
			embeds: [
				{
	...
	private async busyPlayer(track: Track, queue: Queue<Track>, paused: boolean): Promise<MessageEditOptions>
	{
		// Update Queue
		let queueS = "";
		if(queue)
			for(let i = 0; i < queue.length(); i++)
				queueS += `${i+1}. [${queue.at(i).info.title}](${queue.at(i).info.webpage_url})\n`;
